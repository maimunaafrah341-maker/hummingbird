...
| tts alert | 5/5 passed | 1.03 s | `python tts_alert.py --selftest` |
...
**2.1 µs per frame**, so the thing that prevents the flood is far
...
| Model load (cached weights) | 2.45 s |
| Inference alone, one 810×1080 frame in memory | 50 ms  (20.0 fps) |
| Full per-frame loop — frame in, gate decision out | **49 ms  (20.3 fps)** |
...
At **20.0 fps** the 3-of-8 confirmation costs **0.1 s** at best
...
| TTS cache hit | 0.65 ms |
| TTS mp3 size | 52.1 KB |
| TTS cold synthesis (network) | **0.83 s** |
| SMS body | 158 / 160 characters |

Cold synthesis is **1266×** slower than a cache hit and needs the
