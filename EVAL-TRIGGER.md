...
| dossier | 15/15 passed | 0.85 s | `python dossier.py --selftest` |
| webhook dispatch | 17/17 passed | 5.40 s | `python webhook_dispatch.py --selftest` |
| tts alert | 8/8 passed | 1.14 s | `python tts_alert.py --selftest` |
| alert language | 7/7 passed | 2.28 s | `python alert_language.py --selftest` |
| incident service | 12/12 passed | 1.13 s | `python incident_api.py --selftest` |
| escalation watcher | 19/19 passed | 0.77 s | `python escalation_watcher.py --selftest` |
...
| Module | Import |
|---|---|
| `yolo_trigger` | 0.107 s |
| `dossier` | 0.407 s |
| `webhook_dispatch` | 0.129 s |
| `tts_alert` | 0.033 s |
...
| What | Measured |
|---|---|
| `from ultralytics import YOLO` | 3.83 s |
| Model load (cached weights) | 1.52 s |
| Inference alone, one 810×1080 frame in memory | 52 ms  (19.2 fps) |
| Full per-frame loop — frame in, gate decision out | **54 ms  (18.7 fps)** |

Those two rows agree to within 3%, which is run-to-run noise
...
|---|---|
| Resolution | 1280×720 |
| Capture alone | 35 ms  (28.9 fps) |
| Full live path — capture, infer, gate | **63 ms  (15.9 fps)** |
...
| Stage | Measured |
|---|---|
| PDF dossier | 22 ms (3.2 KB) |
| Webhook round trip (local stub) | 1.9 ms |
| TTS cache hit | 0.81 ms |
| TTS mp3 size | 52.1 KB |
| TTS cold synthesis (network) | **0.73 s** |
| SMS body | 158 / 160 characters |

Cold synthesis is **902×** slower than a cache hit and needs the
