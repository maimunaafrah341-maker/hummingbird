...
|---|---|---|---|
| trigger gate | 8/8 passed | 0.20 s | `python yolo_trigger.py selftest` |
| dossier | 10/10 passed | 0.48 s | `python dossier.py --selftest` |
| webhook dispatch | 13/13 passed | 5.86 s | `python webhook_dispatch.py --selftest` |
| tts alert | 5/5 passed | 1.21 s | `python tts_alert.py --selftest` |
| incident rehearsal | 11/11 passed | 1.31 s | `python smoke_test.py incident --no-audio --no-open` |
...
| Module | Import |
|---|---|
| `yolo_trigger` | 0.105 s |
| `dossier` | 0.394 s |
| `webhook_dispatch` | 0.139 s |
| `tts_alert` | 0.015 s |
...
| What | Measured |
|---|---|
| `from ultralytics import YOLO` | 4.52 s |
| Model load (cached weights) | 2.22 s |
| Inference alone, one 810×1080 frame in memory | 67 ms  (14.9 fps) |
| Full per-frame loop — frame in, gate decision out | **69 ms  (14.6 fps)** |

Those two rows agree to within 2%, which is run-to-run noise
on a busy laptop rather than a real difference. That is the
finding: everything the loop does outside the model — unpacking
boxes, the gate decision — costs microseconds against ~67 ms of
...
At **14.6 fps** the 5-frame confirmation costs **0.3 s** before a
...
includes camera capture. A live webcam adds its own read cost: an
earlier run of this loop against the built-in camera measured
**4.9 fps** end to end, roughly 3× slower than the file source
measured here — though that run read from a closed shutter, so it
is a capture-overhead figure, not a detection one. Budget for the
slower number until it is measured on the demo machine with the
lens open.
...
| Stage | Measured |
|---|---|
| PDF dossier | 20 ms (3.2 KB) |
| Webhook round trip (local stub) | 3.2 ms |
| TTS cache hit | 1.13 ms |
| TTS mp3 size | 52.1 KB |
| TTS cold synthesis (network) | **2.23 s** |
| SMS body | 158 / 160 characters |

Cold synthesis is **1966×** slower than a cache hit and needs the
...
- **A real camera.** Frames come from a file. On the machine this was
  measured on the webcam returns flat frames — a closed privacy
  shutter — so the lens-to-model path is unverified here.
